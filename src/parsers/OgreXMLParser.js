    ...
    var OgreXMLParser = namespace.OgreXMLParser = function(assetManager) {
        ...
        meshParser = (function() {
            ...
            function parseMeshXML(xml) {
                ...
                var root, result = [],
                    resultElement, submeshes, nSubmeshes, submesh, meshMatRef, matFile,
                ...
                for (i = nSubmeshes; i--;) {
                    ...
                    if (tGeom.normals.length === 0) {
                        ...
                        console.log("tGeom if");
                    }

                    // result
                    result.push({
                        geometry: tGeom,
                        materialRef: meshMatRef,
                        materialFileRef: matFile
                    });
                }
            ...
            function parseMesh(element, xml, sharedVertexData, geomObject) {
                var elAttributes = element.attributes,
                    operationType, useSharedVertices, faceData, vertexData, geometry;
            ...
            function applyValues(faceData, vertexData, geomObject) {
                ...
                var nVertices = vertexData.nVertices,
                    positions = vertexData.positions,
                    normals = vertexData.normals,
                    hasNormals = normals && normals.length === nVertices,
                    uvs = vertexData.uvs,
                    hasUVs = uvs && uvs.length === nVertices,
        ...
        materialParser = (function() {
                    ...
                    case 'material':
                        {
                            if (line === '}') {
                                // End of material
                                var props = matContext.materialProps,
                                    material;
            ...
            function getTexture(textureRef, matContext) {
                var request = assetManager.requestAsset(textureRef, 'texture'),
                    props = matContext.materialProps,
                    map;
